...
namespace UIAutomation.Framework.Utils
{
    ...
    /// </summary>
    [Serializable]
    public sealed class JavaScriptExecutorsNew : IJavaScriptExecutors
    {
        private ISiteDriver SiteDriver;
        private IBrowserOptions _browserOptions;
        private WebDriverWait _waitPageLoad;

        public void Init(ISiteDriver driver)
        {
            SiteDriver = driver;
           _waitPageLoad = SiteDriver.waitPageLoad;
        }
        ...
        private const string MouseOutTemplate = @"
                ...
                var dispatchMouseEvent = function(target, var_args) { 
                              ...
                              e.initEvent.apply(e, Array.prototype.slice.call(arguments, 1));
                              target.dispatchEvent(e);}
                                dispatchMouseEvent(arguments[0], 'mouseleave', true, true);}
                                else {
                                arguments[0].fireEvent('onmouseleave');}";

        /// <summary>
        /// A JavaScript template executes mouse out of element.
        /// </summary>
        private const string MouseTemplate = @"
                if(navigator.appName == 'Netscape') {
                var dispatchMouseEvent = function(target, var_args) { 
                              var e = document.createEvent('MouseEvents');
                              e.initEvent.apply(e, Array.prototype.slice.call(arguments, 1));
...
var inp = _x(""{0}""); 
        ...
        public object GetUploadFileListObject(string select)
        {
        ...
        /// <returns></returns>
        public int ScrollWidth(string select)
        {
        ...
        /// <param name="select"></param>
        /// <returns></returns>
        public int ClientHeight(string select)
        {
        ...
        /// <param name="select"></param>
        /// <returns></returns>
        public int ClientWidth(string select)
        {
        ...
        /// <param name="select"></param>
        /// <returns></returns>
        public int ScrollHeight(string select)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public bool ExecuteJqueryStatus()
        {
        ...
        /// <param name="select">An element.</param>
        /// <param name="selector">The locating mechanism to use.</param>
        public void ExecuteClick(string select, How selector)
        {
        ...
        /// <param name="select">An element</param>
        /// <param name="selector">The locating mechanism to use</param>
        public void ExecuteContextClick(string select, How selector)
        {
        ...
        public void ExecuteToScrollToView(string elementId, string colIndex)
        {
            SiteDriver.JsExecutor.ExecuteScript(string.Format(JsScrollToView, elementId, colIndex));
        }

        public void ExecuteToScrollToLastLi(string element, bool bottom = true)
        {
            SiteDriver.JsExecutor.ExecuteScript(JsScrollToLastLi, element, bottom);
        }

        public void ExecuteToScrollToSpecificDiv(string element, bool bottom = true)
        {
            SiteDriver.JsExecutor.ExecuteScript(string.Format(JsScrollToSpecificDiv, element, bottom));
            SiteDriver.WaitToLoadNew(200);
        }

        public void ExecuteToScrollToSpecificDivUsingJquery(string element, bool bottom = true)
        {
        ...
        public void ExecuteToScrollToView(string elementClassName)
        {
            SiteDriver.JsExecutor.ExecuteScript(string.Format(JsScrollToViewOfClassName, elementClassName));
        }

        public void ExecuteScrollToMostRight(string elementClassName)
        {
        ...
        /// <param name="select">An element.</param>
        /// <param name="selector">The locating mechanism to use.</param>
        public void ExecuteMouseOut(string select, How selector)
        {
             ExecuteScript(MouseOutTemplate, SiteDriver.FindElement(select, selector));
            //SiteDriver.MouseOver(select, selector, release: true);
        }
        ...
        /// <param name="select">An element.</param>
        /// <param name="selector">The locating mechanism to use.</param>
        public void SendKeys(string strKey, string select, How selector)
        {
            ExecuteScript("arguments[0].value ='" + strKey + "';", Click(select, selector));
        }

        public void SendKeysToInnerHTML(string strKey, string select, How selector)
        {
        ...
        /// <param name="select">An element.</param>
        /// <param name="selector">The locating mechanism to use.</param>
        public void Clear(string select, How selector)
        {
        ...
        /// <param name="selector">The locating mechanism to use.</param>
        /// <returns></returns>
        private IWebElement Click(string select, How selector)
        {
        ...
        public IWebElement FindElementByText(string select, string value)
        {
        ...
        public void SetText(string select, string value)
        {
        ...
        public IWebElement ClickAndGet(string select1, string select2, bool mulitpleScript = false)
        {
            if (mulitpleScript)
            {
        ...
        public IWebElement ClickAndGetWithJquery(string select1, string select2, int interval = 100)
        {
            object element = null;
        ...
        public void ExecuteMouseOut(string select)
        {
            ExecuteScript(MouseOutTemplate, FindElement(select));
        }

        public List<IWebElement> FindWebElements(string select)
        {
            var obj = (IList<IWebElement>)Execute(string.Format(JQueryScriptMultipleResult, select));
            return obj.ToList();
        }

        public List<string> FindElements(string select, string selectSelector = "Text", bool isNull = false, How selector = How.CssSelector)
        {
        ...
        public int FindElementsCount(string select)
        {
        ...
        public bool IsDownloadedFilePresent()
        {
            try
            {
        ...
        public void ClickJQuery(string select)
        {
            Execute(string.Format(JQueryScriptClick, select));
        }

        public void ClickJQueryByText(string select, string text)
        {
            Execute(string.Format(JQueryScriptClickByText, select, text));
        }

        public IWebElement ExecuteFindByXpathClick(string select)
        {
            return (IWebElement)Execute(string.Format(ElementFindByXpath, "\"" + select + "\""));
        }

        public void SetProcCode(string procCodeArrary, string input, string searchIcon)
        {
            Execute(string.Format(JQueryScriptSetProcCode, procCodeArrary, input, searchIcon));
        }
        public void WaitForJqueryStatusCondition()
        {
            try
            {
        ...
        {
            var wait = SiteDriver.WaitTime;
            IWebElement webElement = null;
            wait.Until(driver =>
            {
                try
                {
                    webElement = FindElement(cssLocator);
            ...
            SiteDriver.WaitToLoadNew(200);
            SiteDriver.WebdriverSwitchTo(webElement);
        }

        public List<string> FindElements(string select, How selector, string selectSelector, bool isNull = false)
        {
            if (SiteDriver.FindElementsCount(select, selector) > 15)
                return FindElements(select, selectSelector, isNull, selector);
            var data = new List<string>();
            if (isNull)
                return SiteDriver.FindElements(select, selector).Select(element => element.Text).ToList();
            if (selectSelector.Equals("Text"))
                return SiteDriver.FindElements(select, selector).Where(x => x.Displayed).Select(element => element.Text).ToList();

            if (selectSelector.StartsWith("Attribute"))
                return
                    SiteDriver.FindElements(select, selector)
                        .Where(x => x.Displayed)
                        .Select(element => element.GetAttribute(selectSelector.Replace("Attribute:", "")))
                        .ToList();
            data.Add(SiteDriver.FindElements(select, selector).Where(x => x.Displayed).First().Text.Trim());
            return data;
